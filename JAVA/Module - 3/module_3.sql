...
--
-- Host: 127.0.0.1
-- Generation Time: May 21, 2024 at 08:42 AM
...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `p1` (IN `input_dno` INT)   BEGIN
...
INSERT INTO `company` (`empno`, `ename`, `job`, `mgr`, `hiredate`, `sal`, `comm`, `deptno`) VALUES
(7369, 'SMITH', 'CLERK', 7092, '1980-12-17', 800.00, 0.00, 20),
...
--
DELIMITER $$
CREATE TRIGGER `t1` AFTER INSERT ON `company` FOR EACH ROW BEGIN
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v1`  AS SELECT `e`.`ename` AS `ename`, `e`.`job` AS `job`, `d`.`dname` AS `dname` FROM (`company` `e` join `dept` `d`) WHERE `e`.`deptno` = `d`.`deptno` ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v2`  AS SELECT `c`.`ename` AS `ename`, `c`.`job` AS `job`, `d`.`deptno` AS `deptno`, `d`.`loc` AS `loc` FROM (`company` `c` join `dept` `d`) WHERE `c`.`deptno` = `d`.`deptno` ;
...
--
ALTER TABLE `company`
  ADD CONSTRAINT `company_ibfk_1` FOREIGN KEY (`deptno`) REFERENCES `dept` (`deptno`);
COMMIT;
